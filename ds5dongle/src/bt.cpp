...
int bt_init() {
    queue_init(&send_fifo, sizeof(send_element), BT_SEND_FIFO_DEPTH);
...
void bt_write(const uint8_t *data, const uint16_t len) {
    ...
    const bool was_empty = queue_is_empty(&send_fifo);
    if (!queue_try_add(&send_fifo, &packet)) {
        send_element dropped{};
        if (!queue_try_remove(&send_fifo, &dropped) || !queue_try_add(&send_fifo, &packet)) {
#if ENABLE_VERBOSE
            printf("[L2CAP bt_write] Error: Failed to refresh send FIFO\n");
#endif
            return;
        }
#if ENABLE_VERBOSE
        printf("[L2CAP bt_write] Dropped stale output packet under load\n");
#endif
    }
    if (was_empty) {
        l2cap_request_can_send_now_event(hid_interrupt_cid);
