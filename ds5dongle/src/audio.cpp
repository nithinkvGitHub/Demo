...
void audio_loop() {
    ...
    static float cached_speaker_volume = 1000.0f;
    static uint8_t cached_mute = 0xff;
    static float cached_audio_gain = 0.0f;
    if (cached_speaker_volume != cfg.speaker_volume || cached_mute != mute[0]) {
        cached_speaker_volume = cfg.speaker_volume;
        cached_mute = mute[0];
        cached_audio_gain = mute[0] ? 0.0f : powf(10.0f, cfg.speaker_volume / 20.0f);
    }
    ...
    for (int i = 0; i < nframes; i++) {
        ...
        if (audio_buf_pos == 512 * 2) {
            ...
            if (queue_is_full(&audio_fifo)) {
                static audio_raw_element dropped{};
                queue_try_remove(&audio_fifo, &dropped);
            }
